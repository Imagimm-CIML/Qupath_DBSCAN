...
### 11- To go further : to apply DBSCAN on multiple annotations, load the roi.zip (with name of annotated regions) or draw the annotations in Qupath and apply the groovy script "DBSCAN_loopRegion.groovy" that contain the loop for the regions. The classifier beads and clusters could be saved in a folder named classifiers/object_classifiers (with threshold values) and reused with the command that could be applied in groovy script :  
